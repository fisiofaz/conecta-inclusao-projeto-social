  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        ...
        const userPromise = api.get(`/users/${userId}`);
        //Busca os favoritos do usuário (nova API de Admin)
        const favoritesPromise = api.get(`/favorites/user/${userId}`);
        //Busca as candidaturas do usuário (nova API de Admin)
        const applicationsPromise = api.get(`/candidaturas/user/${userId}`);
  ...
  if (error) {
    return (
      <div className="py-10 text-center text-red-600">
        ...
        <p>{error}</p>
        <Button onClick={() => navigate('/users')} variant="secondary" className="mt-4">
          Voltar para a Lista
        </Button>
      </div>
    );
    ...
    <div className="container p-6 mx-auto my-8 space-y-8">
      ...
      <div className="p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 border-t pt-4 text-gray-700">
          <p><strong>Cidade:</strong> {user.cidade || 'N/A'}</p>
          <p><strong>Estado:</strong> {user.estado || 'N/A'}</p>
          <p><strong>Deficiência:</strong> {user.deficiencia || 'N/A'}</p>
          <p><strong>Data Nasc.:</strong> {user.dataNascimento || 'N/A'}</p>
          <p className="col-span-full"><strong>Bio:</strong> {user.bio || 'N/A'}</p>
        </div>
      </div>

      {/* --- Card 2: Candidaturas do Usuário --- */}
      <div className="p-6 bg-white rounded-lg shadow-md">
        ...
        {applications.length === 0 ? (
          <p className="text-gray-500">Este usuário não se candidatou a nenhuma vaga.</p>
        ) : (
      ...
      <div className="p-6 bg-white rounded-lg shadow-md">
        ...
        {favorites.length === 0 ? (
          <p className="text-gray-500">Este usuário não favoritou nenhum item.</p>
        ) : (
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
